...
public class ThreadPool {
     ...
     * 线程工厂
     */
    private static final ThreadFactory NAMED_THREAD_FACTORY = new ThreadFactoryBuilder().setNameFormat(threadName).build();
...
//        log.error("线程池在运行过程中已完成的任务数量:{}", ThreadPool.getCompletedTaskCount());
//        log.error("线程池里曾经创建过的最大线程数量:{}", ThreadPool.getLargestPoolSize());
//        log.error("获取活动的线程数:{}", ThreadPool.getActiveCount());
//        log.error("线程池的线程数量:{}", ThreadPool.getPoolSize());
//        log.error("线程池需要执行的任务数量:{}

        System.out.println("线程池在运行过程中已完成的任务数量: " + ThreadPool.getCompletedTaskCount());
        System.out.println("线程池里曾经创建过的最大线程数量: " + ThreadPool.getLargestPoolSize());
        ...
        if (threadPool == null) {
            synchronized (ThreadPool.class) {
                if (threadPool == null) {
                    ...
                    threadPool = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, KEEP_ALIVE_TIME, TimeUnit.SECONDS, new LinkedBlockingQueue<>(QUEUE_SIZE), NAMED_THREAD_FACTORY, new ThreadPoolExecutor.AbortPolicy());
    ...
    public static ScheduledExecutorService getScheduledExecutor() {
        if (scheduledExecutor == null) {
            synchronized (ThreadPool.class) {
                if (scheduledExecutor == null) {
                    threadName = "scheduled-pool-%d";
                    scheduledExecutor = new ScheduledThreadPoolExecutor(2, NAMED_THREAD_FACTORY);
                }
            }
        }
